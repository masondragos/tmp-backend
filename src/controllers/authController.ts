...
export const googleAuth = (req: Request, res: Response, next: any) => {
  ...
  const state = req.query.from
    ? encodeURIComponent(req.query.from as string)
    : undefined;
  ...
  (req: Request, res: Response) => {
    try {
      ...
      if (from === "modal") {
        ...
        res.redirect(
          `${frontendURL}/get-quotes?step=sign-in&login_success=true`
        );
      }
    } catch (error) {
      ...
      if (from === "modal") {
        ...
        res.redirect(
          `${frontendURL}/get-quotes?step=sign-in&login_success=false`
        );
      }
    }
  },
...
export const logout = (req: Request, res: Response) => {
  req.logout((err) => {
    ...
    req.session.destroy((err) => {
      ...
      clearAuthCookies(res, "user_auth_token", "user_info");
      clearAuthCookies(res, "employee_auth_token", "employee_info");
      clearAuthCookies(res, "lender_auth_token", "lender_info");
...
export const getUserFromCookie = (req: Request, res: Response) => {
  try {
    // Check for user info in any of the possible cookie names
    const userInfo = req.cookies.user_info || 
                     req.cookies.employee_info || 
                     req.cookies.lender_info;
