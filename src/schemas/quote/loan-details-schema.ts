...
export const fixAndFlipLoanDetailsSchema = z.object({
  ...
  after_repair_property_value: z.string().min(1, "After repair property value is required"),
  ...
  completed_fix_and_flips_2_years: z.string().min(1, "Completed fix & flips in last 2 years is required"),
...
export type FixAndFlipLoanDetailsFormData = z.infer<ReturnType<typeof getFixAndFlipLoanDetailsSchema>>;

export const getFixAndFlipLoanDetailsSchema = (purpose_of_loan: string, has_rehab_funds_requested: string) => {
  if (purpose_of_loan === "purchase") {
    if (has_rehab_funds_requested === "Yes") {
      return fixAndFlipsLoanDetailsNewPurchaseSchema.extend({
        rehab_amount_requested: z.string().min(1, "Rehab amount requested is required"),
      });
    }
...
export type GetFixAndFlipLoanDetailsFormData = z.infer<ReturnType<typeof getFixAndFlipLoanDetailsSchema>>;
;
...
export const dscrRefinanceNoCashFormSchema = z.object({
  purpose_of_loan: z.string(),
  purchase_price: z.string().min(1, "Purchase price is required"),
  property_purchase_date: z.string().min(1, "Property purchase date is required"),
...
export const dscrRefinanceCashOutFormSchema = z.object({
  ...
  property_purchase_date: z.string().min(1, "Property purchase date is required"),
  mortgage_remaining: z.string().min(1, "Mortgage remaining is required"),
  current_property_value: z.string().min(1, "Current property value is required"),
...
export const getDSCRLoanDetailsSchema = ({ purpose_of_loan, has_rehab_funds_requested, has_tenant }: { purpose_of_loan: string, has_rehab_funds_requested: boolean, has_tenant: boolean }) => {
  if (purpose_of_loan === "purchase") {
    ...
    if (has_tenant) {
      schemaToReturn = schemaToReturn.extend({
        how_much_are_they_paying: z.string().min(1, "How much are they paying is required"),
